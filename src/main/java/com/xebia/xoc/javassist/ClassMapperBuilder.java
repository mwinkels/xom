...
public class ClassMapperBuilder<S,T> {
	...
	private CtClass[] handleConstructorArguments(ClassPool classPool,
			CtClass mapperCtClass, Bytecode bytecode, CtClass returnClass)
	...
	private void handleProperties(ClassPool classPool, CtClass mapperCtClass,
			Bytecode bytecode) throws NotFoundException {
	...
	private int addConverterFields(ClassPool classPool, CtClass mapperCtClass, List<? extends AbstractElementMapperBuilder> elements) throws CannotCompileException, NotFoundException {
		int count = 0;
	...
	private void setConverterFields(ClassMapper<S,T> mapperInstance, List<? extends AbstractElementMapperBuilder> elements) throws IllegalAccessException {
		for (AbstractElementMapperBuilder element : elements) {
			if (element.hasConverter()) {
